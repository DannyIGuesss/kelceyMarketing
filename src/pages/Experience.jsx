  ...
  return (
    <>
          ...
          <article className="grid gap-8 md:grid-cols-2 md:items-center">
            {WORK[0].photo ? (
              <img
                ...
                className="h-140 w-full rounded-lg object-cover object-top md:h-160"
              />
            ) : (
              <PhotoPlaceholder label={WORK[0].photoLabel} className="h-80 md:h-90" />
            )}
            <div>
              <p className="text-[11px] uppercase tracking-[0.2em] text-clay">{WORK[0].tag}</p>
              <h2 className="mt-2 font-display text-3xl">{WORK[0].name}</h2>
              <p className="mt-4 text-ink">{WORK[0].blurb}</p>
              <p className="mt-4 text-ink">{WORK[0].blurb2}</p>
            </div>
          </article>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-6 py-16 md:px-10">
        <article className="grid gap-8 md:grid-cols-2 md:items-center md:[&>*:first-child]:order-2">
          {WORK[1].photo ? (
            <img
              src={WORK[1].photo}
          ...
          <div>
            <p className="text-[11px] uppercase tracking-[0.2em] text-clay">{WORK[1].tag}</p>
            <h2 className="mt-2 font-display text-3xl">{WORK[1].name}</h2>
            <p className="mt-4 text-ink">{WORK[1].blurb}</p>
            <p className="mt-4 text-ink">{WORK[1].blurb2}</p>
          </div>
        </article>
      </section>
        ...
        <div className="mx-auto w-full max-w-6xl">
          ...
          <div className="relative mt-12 grid w-full">
            <div className="col-start-1 row-start-1 hidden opacity-99 md:flex">
              <img src={triplePhoto} alt="" className="h-full w-full rounded-lg object-cover" />
            </div>
